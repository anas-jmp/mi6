...
# Compiler and flags
CC = gcc
CFLAGS = -g -Wall -Wextra #-Werror  -fsanitize=address #-D_POSIX_C_SOURCE=200809L
...
PARSING_SRCS = src/$(PARSING_DIR)/parser.c \
               src/$(PARSING_DIR)/ok_parser_p2.c \
               src/$(PARSING_DIR)/builting.c \
               src/$(PARSING_DIR)/tokenize_input.c \
               src/$(PARSING_DIR)/helper.c \
               src/$(PARSING_DIR)/helper2.c \
               src/$(PARSING_DIR)/helper3.c \
               src/$(PARSING_DIR)/free_free.c \
               src/$(PARSING_DIR)/redirection.c \
               src/$(PARSING_DIR)/syntax_error.c \
               src/$(PARSING_DIR)/free_parsed_command.c \
               src/$(PARSING_DIR)/command_handler.c \
               src/$(PARSING_DIR)/helper_function.c \
               src/$(PARSING_DIR)/test_envr.c \

EXECUTION_SRCS = src/$(EXECUTION_DIR)/builtins.c \
                 src/$(EXECUTION_DIR)/read_heredocs.c \
                 src/$(EXECUTION_DIR)/signals.c \
                 src/$(EXECUTION_DIR)/exiting.c \
